...
6. **Merge, Sanitize, and System Prompt**
Each block is refined into its final format for training by performing three key steps:
    * **Merge Consecutive Messages by Role**: Combines consecutive messages from the same role into a single message, preserving the first message's timestamp.
    * **Ensure Proper Role Order**: Ensures that every block starts with the user and ends with the assistant and alternates between roles, trimming any leading assistant messages and trailing user messages.
    * **Prepend System Prompt (Optional)**: If `cfg.system_prompt` is set, a system message is added to the start of every block to guide the model's behavior.

    * **Final Output Format (`train.jsonl`)**
    Each block is serialized as a single JSON line like this:
    ...
    "messages": [
        ...
        {"role": "assistant", "content": ">>> Hey! It's going well, thanks for asking!\n>>> What about you?"}
    ]
    }
    ```
...
*   **AWS S3 (Optional)**: If using S3 for output, ensure you have an AWS account and the necessary permissions to create and write to S3 buckets.
...
Once started, the API server is typically available at `http://localhost:8000`.

## API Reference

| Route                          | Method | Description                                                        | Example Response Body (JSON)                                                                   |
| :----------------------------- | :----- | :----------------------------------------------------------------- | :--------------------------------------------------------------------------------------------- |
| `/process`           | POST   | Submits JSON payload for processing.                             | `{ "status": "queued", "message": "Job accepted...", "run_id": "<uuid>" }`                       |
| `/jobs/{run_id}`     | GET    | Retrieves status and metadata for a specific job (`JobInfo`).      | `{ "run_id": "<uuid>", "status": "COMPLETED", "position_in_queue": 0, "created_at": "...", ...}` |
| `/jobs`              | GET    | Retrieves status and metadata for *all* known jobs.                | `{ "<run_id_1>": { ...JobInfo... }, "<run_id_2>": { ...JobInfo... } }`                          |
| `/queue`      | GET    | Retrieves current queue status, size, and all job statuses.        | `{ "running": false, "queue_size": 0, "jobs": { "<run_id>": { ...JobInfo... } } }`             |
| `/health`     | GET    | Basic health check including S3 connectivity status.               | `{ "status": "healthy", "s3": "connected" }` or `{ "status": "healthy", "s3": "missing" }`      |
...
# 3) Check the status of a specific job using the run_id
