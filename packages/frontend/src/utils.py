...
def show_export_examples() -> None:
    """Show example screenshots and JSON snippets for chat export formats."""
    st.markdown("### 🧾 Telegram Export Formats")
    st.markdown(
        """
        Exports come in two flavors:

        - **All chats** (multi-chat): JSON with top-level `chats.list`  
        - **Single chat**: one dict with a `messages` array

        Resonare supports both.
        """
    )

    col1, col2 = st.columns(2, gap="medium")

    JSON_ALL = textwrap.dedent(
        """
        {
          "about": "...",
          "chats": {
    ...
    with col1:
        st.image("assets/export_all.png", caption="All-chat export")
        st.code(JSON_ALL, language="json")

    with col2:
        st.image("assets/export_individual.png", caption="Single-chat export")
        st.code(JSON_SINGLE, language="json")
